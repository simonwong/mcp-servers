...
// Directory tree API response schema
export const GitLabTreeItemSchema = z.object({
  id: z.string().describe("SHA1 checksum of the file or directory"),
  name: z.string().describe("Name of the file or directory"),
  type: z.string().describe("Entry type, 'tree' for directory, 'blob' for file"),
  path: z.string().describe("Full path of the file or directory"),
  mode: z.string().describe("File mode, '100644' for regular file, '100755' for executable file, '040000' for directory")
});

export const GitLabRepositoryTreeResponseSchema = z.array(GitLabTreeItemSchema)
  .describe("Project directory tree structure containing list of files and directories");
...
export const GetRepositoryTreeSchema = ProjectParamsSchema.extend({
  path: z.string().optional().describe("The path inside the repository. Used to get content of subdirectories"),
  ref: z.string().optional().describe("The name of a repository branch or tag or commit SHA"),
  recursive: z.boolean().optional().describe("Boolean value to get a recursive tree"),
  per_page: z.number().max(100).optional().describe("Number of results per page (default: 20, max: 100)")
});
...
export type GitLabRepositoryTreeResponse = z.infer<typeof GitLabRepositoryTreeResponseSchema>;